...
function Section({ icon, label, children, accent = 'gray', action }: {
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <h4 className="text-xs font-semibold text-muted uppercase tracking-wide flex-1">{label}</h4>
        {action}
      </div>
      {children}
    </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
        ...
        {(subSteps.length > 0 || canEdit) && (
          <Section
            ...
            action={canEdit && subSteps.length > 0 && !editingSubSteps ? (
              <button
                onClick={() => { setSubStepInputs([...subSteps]); setEditingSubSteps(true) }}
                className="flex items-center gap-1 text-[11px] font-medium text-violet-600 hover:text-violet-700 bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/20 px-2 py-0.5 rounded-md transition-colors"
